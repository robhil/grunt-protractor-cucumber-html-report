...
module.exports = (function () {
    ...
    function getStep(step) {
        ...
        stepTemplate = grunt.template.process(template, {
            data: {
                ...
                duration: step.result.duration ? (step.result.duration / 1000000000).toFixed(2) + 's' : 'no data',
                tableOutline: step.rows ? createTableOutline() : '',
    ...
    function getTable (step) {
        var header = step.rows[0].cells,
            data = [],
            tableData = [];

        for (var i = 1; i < step.rows.length; i++) {
            data.push(step.rows[i].cells);
        }
        console.log('step header', header);
        console.log('dane', data);
    }

    function createTableOutline() {
        var table = '<div class="table-outline">';
        return table;
    }
    /**
    ...
    function calculateDuration(duration) {
        var elapsedTime = parseFloat((duration / 1000000000) / 60);
        if ((elapsedTime > 1)) {
            var minutes = Math.floor(elapsedTime),
                seconds = (elapsedTime - minutes) * 60;
            return minutes + 'm ' + seconds.toFixed(0) + 's';
        } else {
            return '0m ' + (elapsedTime * 60).toFixed(0) + 's';
        }
    }
