...
module.exports = (function () {
    ...
    function getStep(step) {
        ...
        stepTemplate = grunt.template.process(template, {
            data: {
                ...
                duration: step.result.duration ? (step.result.duration / 1000000000).toFixed(2) + 's' : '',
    ...
    function calculateDuration(duration) {
        var sec = parseFloat((duration / 1000000000) / 60);
        console.log(duration, sec);
        if ((sec > 1)){
        var integer = Math.floor(sec),
            decimal = (sec - integer) * 60;
            return integer + 'm ' + decimal.toFixed(0) + 's';
        } else {
            return '0m ' + (sec * 60).toFixed(0) + 's';
        }
    }
